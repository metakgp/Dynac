...
function fill_assigner() {
  ...
  for (let i = 0; i < element_fields.length; i++) assigner.set(element_fields[i], []); //map of objects for element holders
  ...
  for (let i = 7; i <= 56; i++) {
    ...
    var temp_desc = oogway.querySelector(`[title="Rich Text Editor, subject${CSS.escape(i)}"]`).contentDocument
      .documentElement;
...
var cw_info = oogway
  .querySelector('[title="Rich Text Editor, research_area"]')
  .contentDocument.documentElement.querySelector("body");
var sknexp = oogway
  .querySelector('[title="Rich Text Editor, skill"]')
  .contentDocument.documentElement.querySelector("body");
